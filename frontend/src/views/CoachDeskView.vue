...
async function refresh() {
  ...
  packages.value = p.data.filter((x: Pkg) => x.status === 'active')
  if (sessions.value[0]) {
    const b = await http.get('/group-bookings', {
      params: { merchant_id: merchantId.value, session_id: sessions.value[0].id },
    })
    bookings.value = b.data.filter((x: Booking) => x.status === 'booked')
  }
}

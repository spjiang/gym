...
const eventQuery = reactive({ q: '', allowed: '' as string })
    ...
    const { data } = await http.get<Page<AccessEvent>>('/access-events', {
      params: {
        ...
        allowed:
          eventQuery.allowed === ''
            ? undefined
            : eventQuery.allowed === '1',
      },
    })
    events.value = data.items
    eventTotal.value = data.total
  } catch (e: unknown) {
    ElMessage.error(e instanceof Error ? e.message : '通行事件加载失败')
  } finally {
...
<template>
  <div>
    ...
    <h3 class="section-title">通行事件</h3>
    <div class="filters">
      <el-input
        v-model="eventQuery.q"
        clearable
        placeholder="会员手机/姓名"
        style="width: 200px"
      ...
      <template v-if="eventDetail">
        <el-descriptions :column="1" border>
          ...
          <el-descriptions-item label="时间">
            {{ eventDetail.created_at?.slice(0, 19).replace('T', ' ') }}

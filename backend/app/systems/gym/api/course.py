...
def list_pt_packages(
    ...
    mid = ctx.resolve_merchant_id(merchant_id)
    filters = [PtPackage.merchant_id == mid]
    ...
    if keyword:
        ...
        filters.append(PtPackage.member_id.in_(mid_sq))
    total = db.scalar(select(func.count()).select_from(PtPackage).where(*filters)) or 0
    rows = list(
        db.scalars(
            select(PtPackage)
            .where(*filters)
...
def list_bookings(
    ...
    mid = ctx.resolve_merchant_id(merchant_id)
    filters = [GroupBooking.merchant_id == mid]
    ...
    total = db.scalar(select(func.count()).select_from(GroupBooking).where(*filters)) or 0
    rows = list(
        db.scalars(
            select(GroupBooking)
            .where(*filters)

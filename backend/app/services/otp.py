...
def send_member_otp(db: Session, *, member_id: int, phone: str) -> str:
    """发送验证码，返回对外提示文案。"""
...
def verify_member_otp(db: Session, *, member_id: int, phone: str, code: str) -> None:
    settings = get_settings()
    mode = settings.member_otp_mode.lower()
    ...
    challenge = db.scalar(
        ...
        .where(
            MemberOtpChallenge.member_id == member_id,
    ...
    if challenge is None:
        # 兼容旧行为：mock 模式下允许直接比对配置码（无发送记录时）
